...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Auto-sync data periodically (Polling)
  ...
  const handleUpdateStatus = async (id, status, catatan = '') => {
    try {
      ...
      // Backend takes care of WhatsApp notification via Fonnte API Listener
          ...
          <div className="nav-grp">
            ...
            {(activeRole === 'master' || activeRole === 'reviewer') && (
              <>
                ...
                <div className={`ni ${activePage === 'proposals' ? 'active' : ''}`} onClick={() => setActivePage('proposals')}>Manajemen Proposal</div>
                <div className={`ni ${activePage === 'verification' ? 'active' : ''}`} onClick={() => setActivePage('verification')}>Verifikasi Evidence</div>
              </>
            )}
            {activeRole === 'user' && (
        ...
        {activePage === 'proposals' && (
          <div className="page-view active content" style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '20px' }}>
            <div className="tc">
              ...
              {renderSearchBar()}
              <table>
                <thead><tr><th>ID</th><th>Pemohon</th><th>Kegiatan</th><th>Tgl Pelaksanaan</th><th>Status</th><th>Aksi</th></tr></thead>
                <tbody>
          ...
          <div className="modal">
            ...
            <div className="mb">
              <div className="ig">
                ...
                <div className="ii"><div className="ik">Instansi</div><div className="iv" style={{ fontWeight: 500, color: 'var(--text)' }}>{selectedProposal.instansi || selectedProposal.user?.instansi || '-'}</div></div>
